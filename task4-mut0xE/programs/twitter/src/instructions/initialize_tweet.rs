...
pub fn initialize_tweet(
    ...
    let signer = &ctx.accounts.tweet_authority;
    ...
    tweet_acc.set_inner(Tweet {
        tweet_author: *signer.key,
        topic,
        content,
        likes: 0,
        dislikes: 0,
...
pub struct InitializeTweet<'info> {
    ...
    #[account(init,payer=tweet_authority,
        space = 8 + Tweet::INIT_SPACE,
        seeds=[topic.as_bytes(),TWEET_SEED.as_bytes(),tweet_authority.key().as_ref()],bump)]
