...
describe("on-chain-vault", async () => {
  ...
  it("Initialize Vault Alice (unlocked)", async () => {
    ...
    assert.strictEqual(
      vaultData.vaultAuthority.toString(),
      alice.publicKey.toString(),
      "Vault authority should be Alice's public key"
    );
    ...
    for (let event of events) {
      if (event.name === "initializeVaultEvent") {
        ...
        assert.strictEqual(
          ...
          vaultAlicePDA.toString(),
          "Event vault should match Alice's vault PDA"
        );
        assert.strictEqual(
          event.data.vaultAuthority.toString(),
          alice.publicKey.toString(),
          "Event vault authority should be Alice"
        );
        assert.strictEqual(
          event.data.locked,
          false,
          "Event locked status should be false"
        );
      }
    }
  ...
  it("Initialize Vault Bob (locked)", async () => {
    ...
    assert.strictEqual(
      ...
      "Vault authority should be Bob's public key"
    );
    ...
    for (let event of events) {
      if (event.name === "initializeVaultEvent") {
        ...
        assert.strictEqual(
          event.data.vault.toString(),
          vaultBobPDA.toString(),
          "Event vault should match Bob's vault PDA"
        );
        assert.strictEqual(
          event.data.vaultAuthority.toString(),
          bob.publicKey.toString(),
          "Event vault authority should be Bob"
        );
        assert.strictEqual(
          event.data.locked,
          true,
          "Event locked status should be true"
        );
      }
    }
    assert.isTrue(logsEmitted, "InitializeVaultEvent should have been emitted");
  ...
  it("Initialize Vault Anatoly (unlocked)", async () => {
    ...
    assert.strictEqual(
      ...
      "Vault authority should be Anatoly's public key"
    );
    assert.strictEqual(vaultData.locked, false, "Vault should be unlocked");
    ...
    for (let event of events) {
      if (event.name === "initializeVaultEvent") {
        ...
        assert.strictEqual(
          event.data.vault.toString(),
          vaultAnatolyPDA.toString(),
          "Event vault should match Anatoly's vault PDA"
        );
        assert.strictEqual(
          event.data.vaultAuthority.toString(),
          anatoly.publicKey.toString(),
          "Event vault authority should be Anatoly"
        );
        assert.strictEqual(
          event.data.locked,
          false,
  ...
  it("Cannot initialize vault for someone else", async () => {
    ...
    try {
      ...
      assert.isTrue(
        error.toString().includes("Error"),
        "Should fail with signature mismatch error"
      );
    }
    assert.strictEqual(
      flag,
      "Failed",
      "Initializing vault for someone else should fail"
    );
  });
  ...
  it("Deposit to Alice's vault", async () => {
    ...
    assert.isTrue(
      ...
      "User balance should decrease after deposit"
    );
    ...
    for (let event of events) {
      if (event.name === "depositEvent") {
        ...
        assert.strictEqual(
          ...
          "Event amount should match deposit amount"
        );
        assert.strictEqual(
          event.data.user.toString(),
          alice.publicKey.toString(),
          "Event user should be Alice"
        );
        assert.strictEqual(
          event.data.vault.toString(),
          vaultAlicePDA.toString(),
          "Event vault should be Alice's vault"
        );
      }
    }
  ...
  it("Cannot deposit to locked vault (Bob's vault)", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Depositing to locked vault should fail"
    );
  });
  ...
  it("Cannot deposit to non-existent vault", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Depositing to non-existent vault should fail"
    );
  });
  ...
  it("Cannot deposit more than user balance", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Depositing more than user balance should fail"
    );
  });
  ...
  it("Toggle lock on Bob's vault (unlock it)", async () => {
    ...
    assert.strictEqual(
      vaultData.locked,
      false,
      "Vault should be unlocked after toggle"
    );
    ...
    for (let event of events) {
      if (event.name === "toggleLockEvent") {
        ...
        assert.strictEqual(
          event.data.locked,
          false,
          "Event locked status should be false after unlock"
        );
      }
    }
  ...
  it("Now can deposit to Bob's vault after unlocking", async () => {
    ...
    assert.isTrue(
      vaultBalanceAfter > vaultBalanceBefore,
      "Vault balance should increase after deposit to unlocked vault"
    );
  });
  ...
  it("Toggle lock on Alice's vault (lock it)", async () => {
    ...
    assert.strictEqual(
      vaultData.locked,
      true,
      "Vault should be locked after toggle"
    );
    ...
    for (let event of events) {
      if (event.name === "toggleLockEvent") {
        ...
        assert.strictEqual(
          event.data.locked,
          true,
          "Event locked status should be true after lock"
        );
      }
    }
    assert.isTrue(logsEmitted, "ToggleLockEvent should have been emitted");
  ...
  it("Withdraw from Bob's vault", async () => {
    ...
    assert.isTrue(
      ...
      "Authority balance should increase after withdrawal"
    );
    ...
    for (let event of events) {
      if (event.name === "withdrawEvent") {
        ...
        assert.strictEqual(
          ...
          "Event amount should match withdrawal amount"
        );
        assert.strictEqual(
          event.data.vaultAuthority.toString(),
          bob.publicKey.toString(),
        ...
        assert.strictEqual(
          event.data.vault.toString(),
          vaultBobPDA.toString(),
          "Event vault should be Bob's vault"
        );
      }
    }
  ...
  it("Cannot withdraw from Alice's locked vault", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Withdrawing from locked vault should fail"
    );
  });
  ...
  it("Cannot withdraw from vault without authority", async () => {
    ...
    try {
      ...
      assert.isTrue(
        error.toString().includes("Error"),
        "Should fail due to seeds constraint - wrong authority"
      );
    }
    assert.strictEqual(
      flag,
      "Failed",
      "Withdrawing without proper authority should fail"
    );
  });
  ...
  it("Cannot withdraw more than vault balance", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Withdrawing more than vault balance should fail"
    );
  });
  ...
  it("Cannot toggle lock without authority", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Toggling lock without proper authority should fail"
    );
  });
  ...
  it("Cannot toggle lock on non-existent vault", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Toggling lock on non-existent vault should fail"
    );
  });
  ...
  it("Multiple deposits and withdrawals work correctly", async () => {
    ...
    const vaultBalanceFinal = await provider.connection.getBalance(
      vaultAnatolyPDA
    );
    assert.isTrue(
      vaultBalanceFinal < vaultBalanceAfter,
      "Vault balance should decrease after withdrawal"
    );
  });
  ...
  it("Cross-user deposits: Alice deposits into Bob's vault", async () => {
    ...
    assert.isTrue(
      vaultBalanceAfter > vaultBalanceBefore,
      "Bob's vault balance should increase when Alice deposits"
    );
    assert.isTrue(
      aliceBalanceAfter < aliceBalanceBefore,
      "Alice's balance should decrease when she deposits"
    );
    ...
    for (let event of events) {
      if (event.name === "depositEvent") {
        ...
        assert.strictEqual(
          event.data.user.toString(),
          alice.publicKey.toString(),
          "Event user should be Alice (the depositor)"
        );
        assert.strictEqual(
          event.data.vault.toString(),
          vaultBobPDA.toString(),
    ...
    assert.isTrue(
      logsEmitted,
      "DepositEvent should have been emitted for cross-user deposit"
    );
  });
  ...
  it("Cross-user deposits: Bob deposits into Anatoly's vault", async () => {
    ...
    assert.isTrue(
      vaultBalanceAfter > vaultBalanceBefore,
      "Anatoly's vault balance should increase when Bob deposits"
    );
    assert.isTrue(
      bobBalanceAfter < bobBalanceBefore,
      "Bob's balance should decrease when he deposits"
    );
    ...
    for (let event of events) {
      if (event.name === "depositEvent") {
        ...
        assert.strictEqual(
          event.data.user.toString(),
          bob.publicKey.toString(),
          "Event user should be Bob (the depositor)"
        );
        assert.strictEqual(
          event.data.vault.toString(),
          vaultAnatolyPDA.toString(),
          "Event vault should be Anatoly's vault"
        );
      }
    }
    assert.isTrue(
  ...
  it("Cross-user deposits: Anatoly deposits into Alice's vault (after unlocking)", async () => {
    ...
    const anatolyBalanceBefore = await provider.connection.getBalance(
      anatoly.publicKey
    );
    ...
    const anatolyBalanceAfter = await provider.connection.getBalance(
      anatoly.publicKey
    );
    ...
    assert.isTrue(
      vaultBalanceAfter > vaultBalanceBefore,
      "Alice's vault balance should increase when Anatoly deposits"
    );
    assert.isTrue(
      anatolyBalanceAfter < anatolyBalanceBefore,
      "Anatoly's balance should decrease when he deposits"
    );
    ...
    for (let event of events) {
      if (event.name === "depositEvent") {
        ...
        assert.strictEqual(
          event.data.user.toString(),
          anatoly.publicKey.toString(),
          "Event user should be Anatoly (the depositor)"
        );
        assert.strictEqual(
          event.data.vault.toString(),
          vaultAlicePDA.toString(),
  ...
  it("Cannot cross-deposit into locked vault", async () => {
    ...
    assert.strictEqual(
      flag,
      "Failed",
      "Cross-depositing into locked vault should fail"
    );
  });
  ...
  it("Only vault authority can withdraw (not depositors)", async () => {
    ...
    try {
      ...
      assert.isTrue(
        error.toString().includes("Error"),
        "Should fail due to seeds constraint - Bob is not vault authority"
      );
    }
    assert.strictEqual(
      flag,
      "Failed",
      "Non-authority should not be able to withdraw even if they deposited"
    );
